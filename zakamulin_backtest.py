...
Tested on SPY, QQQ, IWM | Monthly rebalance | 0.10% round-trip cost on signal change
"""
...
RESULTS_DIR   = Path(r"C:\Users\rmena\OneDrive\Documents\Claude\Projects\Mr. Buffet\vibe-trading-skills\results")
ASSETS        = ["SPY", "QQQ", "IWM"]
...
raw = {sym: fetch_monthly_closes(sym) for sym in ASSETS}
print(f"  Monthly periods per asset: { {k: len(v) for k,v in raw.items()} }")
...
def run_backtest(prices: pd.Series, signals: pd.Series, name: str) -> dict:
    ...
    position    = df["signal"].shift(1).fillna(0)  # execute next month

    # transaction cost: half round-trip on each side of a trade
    ...
    if n_trades > 0:
        # win rate: months where we were long and market went up
...
for sym in ASSETS:
    ...
    # align buy-and-hold to same date range as strategy equity
    common_start = results["A_SMSM"]["equity"].index[0]
    bh_aligned   = bh[bh.index >= common_start]
    bh_aligned   = bh_aligned / bh_aligned.iloc[0]  # rebase to 1.0 at same start
